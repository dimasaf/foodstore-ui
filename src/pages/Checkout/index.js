...
const columns = [
	{
		...
		accessor: item => <div className="flex items-center">
							<img src={`${config.api_host}/upload/${item.image_url}`} width={48} alt={item.name}/>{item.name}
						</div>
	},
	{
		Header: 'Jumlah',
...
const addressColumns = [
	...
	return <div>
				{alamat.nama} <br/>
				<small>	{alamat.provinsi}, {alamat.kabupaten}, {alamat.kecamatan}, {alamat.kelurahan} <br/> {alamat.detail}</small>
			</div>
	}
	}
]
					...
					{!data.length && status === 'success' ?
						...
						<Responsive desktop={2} tablet={2} mobile={2}>
							...
							<Button
								onClick={_ => setActiveStep(activeStep - 1)}
								color="gray"
								iconBefore={<FaArrowLeft/>}> Sebelumnya
							</Button>
							</div>
							<div className="text-right">
								<Button
									...
									color="red"
									iconAfter={<FaArrowRight/>}> Selanjutnya
								</Button>
							</div>
						</Responsive>
				</div>  : null }
